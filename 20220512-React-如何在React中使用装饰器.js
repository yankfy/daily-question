...
方式一：经过 eject 后在 package.json 中的 plugins 中配置
...
在 babel 对象处进行插件的配置，将@babel/plugin-proposal-decorators添加到 plugins 后
...
方式二：安装 babel 插件在 babelrc 中配置
在使用这种装饰器方式时，需要对 create-react-app 做一些配置，它默认是不支持装饰器模式的，你需要对项目做一些配置

在 create-react-app 根目录中终端下使用npm run eject，这条命令主要是将我们的配置项做一个反向输出，暴露出隐藏的 webpack 配置项，这样可以项目进行修改了的，注意它是不可逆的
